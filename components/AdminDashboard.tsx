      ...
      {tab === 'posts' && (
        <div className="bg-gray-900 border border-gray-800 rounded-[2.5rem] overflow-hidden shadow-2xl animate-in fade-in duration-500">
          <div className="p-8 border-b border-gray-800 flex justify-between items-center">
            <h3 className="text-xl font-black text-white uppercase tracking-tighter">Article Library</h3>
            <button onClick={() => setEditingPost({})} className="bg-indigo-600 hover:bg-indigo-500 px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest text-white shadow-lg shadow-indigo-600/30">New Article</button>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-800/20 border-b border-gray-800">
                  <th className="p-6 text-[10px] font-black text-gray-500 uppercase tracking-widest">Post Title</th>
              ...
              <tbody className="divide-y divide-gray-800/50">
                {posts.map(post => (
                  <tr key={post.id} className="hover:bg-gray-800/10 transition-colors group">
                    <td className="p-6">
                    ...
                    <td className="p-6 text-right space-x-2">
                      ...
                      <button 
                        onClick={() => { if(confirm('Delete permanently?')) onUpdatePosts(posts.filter(p => p.id !== post.id)) }} 
