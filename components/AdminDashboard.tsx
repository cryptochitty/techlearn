  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-center bg-gray-900/40 p-6 rounded-[2rem] border border-gray-800 gap-4">
        ...
        <button onClick={onLogout} className="px-6 py-2 border border-red-500/20 text-red-500 text-[10px] font-black uppercase tracking-widest rounded-full hover:bg-red-500 hover:text-white transition-all">Terminate Session</button>
      </div>
                ...
                {posts.map(post => (
                  <tr key={post.id} className="hover:bg-gray-800/10 transition-colors">
                    <td className="p-6">
                      ...
                      <div className="text-[9px] font-black text-indigo-500 uppercase tracking-widest">{post.category}</div>
                    </td>
           ...
           <form onSubmit={handleSaveSettings} className="space-y-6">
             <div className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full bg-black border border-gray-800 rounded-xl px-4 py-3 text-white font-bold outline-none focus:ring-1 focus:ring-indigo-500 transition-all" 
      ...
      {editingPost && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-gray-900 border border-gray-800 rounded-[3rem] w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95 duration-300">
            <div className="p-8 border-b border-gray-800 flex justify-between items-center">
              <h4 className="text-xl font-black text-white lowercase">article<span className="text-indigo-600">.</span>editor</h4>
              <button onClick={() => setEditingPost(null)} className="text-gray-500 hover:text-white transition-colors">
            ...
            <form onSubmit={handleSavePost} className="p-8 space-y-8 overflow-y-auto">
              <div className="grid grid-cols-2 gap-8">
                <div className="space-y-2">
                  ...
                  <input type="text" className="w-full bg-black border border-gray-800 rounded-xl px-4 py-3 text-white font-bold outline-none focus:ring-1 focus:ring-indigo-500" value={editingPost.title || ''} onChange={e => setEditingPost({...editingPost, title: e.target.value})} />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Sector</label>
                  <select className="w-full bg-black border border-gray-800 rounded-xl px-4 py-3 text-white font-bold outline-none focus:ring-1 focus:ring-indigo-500" value={editingPost.category || ''} onChange={e => setEditingPost({...editingPost, category: e.target.value})}>
